...
void enter(){
    if(curr_char_ptr < KEY_BUF_SIZE){
        ...
        terminal_out_write_((uint8_t*)seq_enter, 3);
    }
    if(read_test_mode == 1){
        ...
        terminal_out_write_(testbuf, num_read);
        if(testbuf[0]=='`'){
            read_test_mode = 0;
        }
    }   
...
void backspace(){
    if(curr_char_ptr>0 && kbd_buf[curr_char_ptr-1]!='\n'){
        ...
        terminal_out_write_((uint8_t*)seq_backspace,3);
    }
...
void ctrl_l(){
    ...
    terminal_out_write_((uint8_t*)seq_ctrl_l,3);
}
